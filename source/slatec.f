...
C     INCFD.LT.1 RETURN.
      IERR = -2
c$$$      CALL XERMSG ('SLATEC', 'DPCHSP', 'INCREMENT LESS THAN ONE', IERR,
c$$$     +   1)
      call abrt(ierr)
      RETURN
C
...
C     IC OUT OF RANGE RETURN.
      IERR = IERR - 3
c$$$      CALL XERMSG ('SLATEC', 'DPCHSP', 'IC OUT OF RANGE', IERR, 1)
      call abrt(ierr)
      RETURN
C
 5007 CONTINUE
C     NWK TOO SMALL RETURN.
      IERR = -7
c$$$      CALL XERMSG ('SLATEC', 'DPCHSP', 'WORK ARRAY TOO SMALL', IERR, 1)
      call abrt(ierr)
      RETURN
C
 5008 CONTINUE
...
C   *** ARE EQUAL, WHICH SHOULD ALREADY HAVE BEEN CAUGHT (IERR=-3). ***
      IERR = -8
c$$$      CALL XERMSG ('SLATEC', 'DPCHSP', 'SINGULAR LINEAR SYSTEM', IERR,
c$$$     +   1)
      call abrt(ierr)
      RETURN
C
...
C   *** THIS CASE SHOULD NEVER OCCUR ***
      IERR = -9
c$$$      CALL XERMSG ('SLATEC', 'DPCHSP', 'ERROR RETURN FROM DPCHDF',
c$$$     +   IERR, 1)
      call abrt(ierr)
      RETURN
