...
## 概要
`rismical-bind.py` は、**3D-RISM** 理論を用いてリガンドの結合自由エネルギーを計算するための Python 自動化スクリプトです。**MM-PBSA** 法と同様のワークフローを実装しており、分子力学 (MM) 相互作用エネルギーと、RISMiCal から得られる溶媒和自由エネルギーを組み合わせます。
