c----------------------------------------------------------------------
c           Convert SDF to RDF                                         
c----------------------------------------------------------------------
c
c     Usage: sdf2rdf.x [guvfile] [center list file]
c
c     Format of center list file
c     1 Number of centers, r_min, r_max, rdelta
c     2 [name] [x] [y] [z]
c     3 [name] [x] [y] [z]
c     (Repeat until the number of center points is reached.)
c
c     r_min: Minimum distance of RDF
c     r_max: Maximum distance of RDF
...
c----------------------------------------------------------------------
      ...
      real*8, allocatable :: guv(:,:),cxyz(:,:)
      character*12, allocatable :: cname(:)
     ...
     .      costhe(maxnod), sinthe(maxnod)                             
      character*80  guvfile,inpfile,rdffile
      dimension gr(maxnv),rv(3), gvv8(maxnv)
      dimension igc(3,2),delc(3,2)

c----------------------------------------------------------------------
      call  getarg (1,guvfile)                                       
      call  getarg (2,inpfile)                                       
c
c     read center point file
c
      ift=45
      open (ift,file=inpfile,status='old',err=9000)
        read (ift,*,err=9000)  ncenter,rmin,rmax,rdelta
        allocate (cxyz(3,ncenter))
        allocate (cname(ncenter))
        
        do i=1,ncenter
          read (ift,*,err=9000)  cname(i),(cxyz(j,i),j=1,3)
        enddo                                                           
      close (ift,err=9000)  
c
c     read guv file
c
      open (ift,file=guvfile,form="unformatted",access="direct",recl=4)

      read(ift,rec=1) nvuq
      read(ift,rec=2) ngrid3d(1)
      read(ift,rec=3) ngrid3d(2)
      read(ift,rec=4) ngrid3d(3)
      close(ift)
      ng3d=ngrid3d(1)*ngrid3d(2)*ngrid3d(3)

      allocate (guv(ng3d,nvuq))
      open (ift,file=guvfile,form="unformatted",access="direct",recl=8)
      read(ift,rec=3) box(1)
      read(ift,rec=4) box(2)
      read(ift,rec=5) box(3)
      read(ift,rec=6) cshift(1)
      read(ift,rec=7) cshift(2)
      read(ift,rec=8) cshift(3)

      do iv=1,nvuq
         do ig=1,ng3d
            irec=(iv-1)*ng3d+ig+8
            read(ift,rec=irec)  guv(ig,iv)
         enddo
      enddo
      close(ift)
c
...
c
      call angnodedat(nod,phi,theta,angwei)
c
      do inod=1,nod                                                     
...
c
      imin=int(rmin/rdelta)
      imax=int(rmax/rdelta)+1

      do ip=1,ncenter

         idot=index(guvfile,".",back=.true.)
         rdffile=trim(guvfile(1:idot))//trim(cname(ip))//".rdf"

         write(*,'(A7,1x,A12,2x,3f14.5,2x,A3,A80)') 
...
c                                                                        
 8000 stop                                                              
 9000 write(*,*) "I/O error. Invalid input file/s."
      goto 8000
      end
c----------------------------------------------------------------
      subroutine angnodedat(nod,phi,theta,angwei)
      
      parameter (maxnod=1000)                                
      real*8   phi(maxnod), theta(maxnod), angwei(maxnod)
c
      nod=700
