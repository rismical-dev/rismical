...
c
c     Usage: parm2rismcal [parm7] [pdb|crd] ([format])
c
c     Note: crd file should be ASCII format
c          (corresponding amber option is ntxo=1)
c
c     format=0  CUDA version
c     format=1  RISMiCal  (default)
c
      program main
...
c
      ...
      if (nargc.lt.2) then
         ...
         write(0,*) "Usage: parm2rismcal [parm7] [pdb|crd] ([format])"
         stop
      endif
...
c     check coordinate format
      np=scan(pdbfile,".",back=.true.)
      char8=pdbfile(np+1:np+9)
